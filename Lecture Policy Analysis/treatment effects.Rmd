...
$$E[ Y_i(1) - Y_i(0)|w_i=1] = E[Y_i(1)|w_i=1]-E[Y_i(0)|w_i=1] $$
...
random assignment assumption is now that $x_i$ is independent of $[u_i(0),u_i(1)]$, even though $u_i$ depends on $x_i$
...
Written another way:
$$(Y_1, Y_0) \bot w_i | X_i$$ (selection on observables/unconfoundness (as discussed earlier))
...
$$\tau = E(Y_1 - Y_0) = \int E[Y_1-Y_i|X]dP(X) $$
...
$$\tau_{ATET} = E(Y_1 - Y_0|w_i=1) = \int E[Y_1-Y_i|X]dP(X) $$
...
$$\tau  = \sum^k_{i=1}(\bar{Y}^k_1-\bar{Y}^k_0 \dot (\frac{N_k}{N})$$
Where $N_k$ is the number of observations, $N_k_1$ is the number of treated observations
...
You need to think about the assignment mechanism - what impacts selection into going to catholic school. Let's think about this now and make suggestions on what coudld be included.
...
What we got to do is find pairs of observations that have similar propensity scores, but only differ in their treatment status. In other
words, match pairs. We'll use the funciton MatchIt to do this.
